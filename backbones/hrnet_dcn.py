...
model_urls = {
    ...
    # 'hrnet18': '/home/math-tr/jzy/DB-master/backbones/pre_module_pth/hrnetv2_w18_imagenet_pretrained.pth',
...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
        ...
        # 高分辨率网络参数。分支数量，块，块的数量，输入（输出）通道的数量，融合模块，多尺度（规模输出），归一�?
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        # 以hrnet18为例
        # stage 1
        self.stage1_cfg = cfg['STAGE1']  # 配置文件里STAGE1
        ...
        self.layer1 = self._make_layer(block, 64, num_channels, num_blocks)  # 经过4（blocks）层的卷�?
        stage1_out_channel = block.expansion * num_channels  # 输出通道=4*64=256
        # expansion 与block的类型有关：BasicBlock的expansion=1，Bottleneck的expansion=4
...
############原始henet里的_make_layer�?###########
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, dcn=None):
        downsample = None
        ...
        layers = []
        layers.append(block(inplanes, planes, stride, downsample, norm_layer=self.norm_layer, dcn=dcn))
        inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(inplanes, planes, norm_layer=self.norm_layer,  dcn=dcn))
    ...
    def _make_stage(self, layer_config, num_inchannels,
        ...
        for i in range(num_modules):
            ...
            # print("输入通道�?, num_inchannels,num_channels)
    ...
    def forward(self, x):
        ...
        x = self.bn2(x)
        x = self.relu(x)
        x = self.layer1(x)  # 这里包含四个bottleneck
        ...
        # 可以在这里得到最终的输出，然后把它们传给seg_detector.py下的c2, c3, c4, c5 = features
        x = torch.cat([x[0], x1, x2, x3], 1)

        x = self.last_layer(x)
        # print("----------------", x.shape)        # torch.Size([2, 256, 160, 160])
...
def _hrnet(arch, pretrained, progress, **kwargs):
    # print("----------------------------------------", arch)    # 只得�?dcn_hrnet18，这是对的，我需要加载名字为deformable_hrnt18的模块�?
    try:
    ...
    model = HighResolutionNet(MODEL_CONFIGS[arch],
                            ...
                            deformable_groups=1,
                            fallback_on_stride=False), **kwargs)
    if pretrained:
        model_url = model_urls[arch]
        #print("下载模型路径�?, model_url)
        state_dict = load_state_dict_from_url(model_url,
                                              progress=progress)
        model.load_state_dict(state_dict, strict=False)
    # else:
    #     print("load pretrained model...")
    #     model.load_state_dict(torch.load(r'/home/math-tr/jzy/DB-master/backbones/pre_module_pth/hrnet_w18_small_model_v1.pth'))
    # print(model)
    return model

###added      with dcn##########
def _dcn_hrnet(arch, pretrained, progress, **kwargs):
    # print("----------------------------------------", arch)    # 只得�?dcn_hrnet18，这是对的，我需要加载名字为deformable_hrnt18的模块�?
    try:
        from config.hrnet_config import MODEL_CONFIGS
    except ImportError:
...
def dcn_hrnet18(pretrained=False,progress=True, **kwargs):
    r"""deformable_HRNet-18 model
    """
    return _dcn_hrnet('dcn_hrnet18',pretrained, progress,**kwargs)
...
def dcn_hrnet48(pretrained=False,progress=True, **kwargs):
    r"""deformable_HRNet-18 model
    """
    return _dcn_hrnet('dcn_hrnet48',pretrained, progress,**kwargs)