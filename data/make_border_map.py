...
class MakeBorderMap(Configurable):
    ...
    def __call__(self, data, *args, **kwargs):
        ...
        canvas = canvas * (self.thresh_max - self.thresh_min) + self.thresh_min     # 640*640
        # print("##########################", canvas.shape)
        data['thresh_map'] = canvas
        data['thresh_mask'] = mask
        
        ########adde    t_mask ########
        data['text_map'] = canvas*mask        # ����Ӧ����+
        # data['text_map'] = canvas + mask        # ����Ӧ����+
        # print("=======================", data)
        return data
    ...
    #     polygon_shape = Polygon(polygon)
    #     distance = polygon_shape.area * \
    #         (1 - np.power(self.shrink_ratio, 2)) / polygon_shape.length      # 偏移量d的求�?        
    #         subject = [tuple(l) for l in polygon]
    #         padding = pyclipper.PyclipperOffset()
    #         padding.AddPath(subject, pyclipper.JT_ROUND,
    #                     pyclipper.ET_CLOSEDPOLYGON)
    #     padded_polygon = np.array(padding.Execute(distance)[0])                 # padded后的多边形的�?        # print("=============", padded_polygon)
    #     # print(padded_polygon[:, 0])
    #     cv2.fillPoly(mask, [padded_polygon.astype(np.int32)], 1.0)              # 填充多边形[padded_polygon.astype(np.int32)]

    #     xmin = padded_polygon[:, 0].min()       # 取第一列的值的最小值，x�?        xmax = padded_polygon[:, 0].max()
    #     ymin = padded_polygon[:, 1].min()       # 取第二列的值的最小值，y�?        ymax = padded_polygon[:, 1].max()
    #     width = xmax - xmin + 1                 # 多边形的w
    #     height = ymax - ymin + 1                # 多边形的h

    #     # 归一�?        polygon[:, 0] = polygon[:, 0] - xmin    # 原始多边形的x值减去最小值xmin
    #     polygon[:, 1] = polygon[:, 1] - ymin    # 原始多边形的y值减去最小值ymin
