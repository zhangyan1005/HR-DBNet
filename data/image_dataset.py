    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            with open(self.data_list[i], 'r') as fid:
                image_list = fid.readlines()        # 这里得到的是数据路径下的所有图片的名字
                # print("this is image_list=========== ", image_list)
            # ###IC2015###
            # if self.is_training:
            #     image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list]
            #     gt_path=[self.data_dir[i]+'/training_gts/'+timg.strip().split('.')[0]+'.txt' for timg in image_list]
            ...
            if self.is_training:
                image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list]
                gt_path=[self.data_dir[i]+'train_gts/'+timg.strip().split('.')[0]+'.jpg'+'.txt' for timg in image_list]
            # td500
            # if self.is_training:
            #     image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list]
            #     gt_path=[self.data_dir[i]+'train_gts/' + timg.strip().split('.')[0]+'.gt' for timg in image_list]
            # CTW1500
            # if self.is_training:
            #     image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list]
            #     gt_path=[self.data_dir[i]+'train_gts/' + timg.strip().split('.')[0]+'.txt' for timg in image_list]
                ...
                if 'TD500' in self.data_list[i] or 'total_text' in self.data_list[i]:
                    ...
                    # print("gt_path1111111111", gt_path )
                else:
