...
class L1BalanceCELoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5):
        ...
        # # # #########added#########
        # # text_loss, l2_metric = self.l1_loss(pred['binary'], batch['gt'], batch['text_map'])
        # # metrics = dict(text_loss=text_loss)
        # # #########added#########
        # # print("-----------", metrics)
        # if 'thresh' in pred:
        #     l1_loss, l1_metric = self.l1_loss(pred['thresh'], batch['thresh_map'], batch['thresh_mask'])
        #     # print("1"*12, l1_loss, l1_metric)
        #     dice_loss = self.dice_loss(pred['thresh_binary'], batch['gt'], batch['mask'])
        #     metrics['thresh_loss'] = dice_loss
        #     # loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale

        #     # # #########added#########
        #     # loss = dice_loss + self.l1_scale * text_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale
        #     metrics.update(**l1_metric)
        #     # print(metrics)
        # else:
        #     loss = bce_loss
    ...
    def forward(self, pred, batch):
        ...
        metrics = dict(bce_loss=bce_loss)
        # #########added#########
        text_loss, l2_metric = self.l1_loss(pred['binary'], batch['text_map'], batch['mask'])
        metrics['text_loss'] = text_loss
        # #########added#########
        if 'thresh' in pred:
            # text_loss, l2_metric = self.l1_loss(pred['binary'], batch['text_map'], batch['thresh_mask']) # p
            # metrics['text_loss'] = text_loss
            l1_loss, l1_metric = self.l1_loss(pred['thresh'], batch['thresh_map'], batch['thresh_mask']) #  f1 = 1p+ 9text_map
            ...
            # #########added#########
            loss = dice_loss*self.bce_scale + self.l1_scale * text_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale
            metrics.update(**l1_metric)
            # print(metrics)
        else:
            loss = bce_loss
...
#             metrics['thresh_loss'] = dice_loss
#             # loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale
#             loss = dice_loss*self.bce_scale + self.l1_scale * l1_loss + bce_loss * self.bce_scale
#             metrics.update(**l1_metric)
#         else:
#             loss = bce_loss
#         return loss, metrics
